...
def scroll_and_select_group(page, username, group_targets):
    ...
    group_tab_selector = 'xpath=//div[@role="tab" and (contains(text(), "群消息") or contains(text(), "群聊"))]'
    ...
    logger.debug(f"账号 {username} 开始查找目标群聊列表")
    logger.debug(f"账号 {username} 目标群聊列表: {group_targets}")
    ...
    found_targets = set()
    remaining_targets = set(group_targets)
    empty_scroll_count = 0
    MAX_EMPTY_SCROLLS = 10

    while True:
        target_elements = page.locator(group_item_selector).all()
        prev_found_count = len(found_targets)
        ...
        for element in target_elements:
            try:
                ...
                group_name = span.inner_text()

                if group_name in found_targets:
                    continue
                found_targets.add(group_name)
                ...
                if group_name in group_targets:
                    ...
                    remaining_targets.discard(group_name)
                    if len(remaining_targets) == 0:
                        logger.debug(f"账号 {username} 所有目标群聊均已找到，停止搜索")
                        return
                    break
            except Exception as e:
                traceback.print_exc()
            ...
            if page.locator(no_more_selector).count() > 0:
                logger.info(f"账号 {username} 检测到'没有更多了'标志，已到达底部")
                if len(remaining_targets) > 0:
                    logger.warning(f"账号 {username} 搜索结束，仍有以下群聊未找到: {remaining_targets}")
                break

            if empty_scroll_count >= MAX_EMPTY_SCROLLS:
                logger.warning(f"账号 {username} 连续 {MAX_EMPTY_SCROLLS} 次滚动未发现新群聊，判定已到达底部")
                if len(remaining_targets) > 0:
                    logger.warning(f"账号 {username} 搜索结束，仍有以下群聊未找到: {remaining_targets}")
                break

            if page.locator(loading_selector).count() > 0:
                logger.debug(f"账号 {username} 群聊列表正在加载中...")
                time.sleep(1.5)

            scrollable_element = page.locator(scrollable_group_selector).element_handle()
            if scrollable_element:
                ...
                if scroll_top_before == scroll_top_after:
                    ...
                    logger.debug(f"账号 {username} scrollTop 未变化，可能已到底 (空滚动计数: {empty_scroll_count}/{MAX_EMPTY_SCROLLS})")
                else:
                    logger.debug(f"账号 {username} 滚动群聊列表 (scrollTop: {scroll_top_before} -> {scroll_top_after})")

                time.sleep(1.5)
            else:
                logger.error(f"账号 {username} 未找到群聊滚动容器，退出")
                break
...
def send_message_to_chat(page, username, target_name, is_group=False):
    ...
    time.sleep(2)
    logger.debug(f"账号 {username} 给{chat_type} {target_name} 发送消息完成")
...
def do_user_task(browser, username, cookies, targets, group_targets=None):
        ...
        # 处理私聊好友
        if targets:
            logger.debug(f"账号 {username} 开始发送私聊消息")
            for friend_name in scroll_and_select_user(page, username, targets):
                logger.debug(f"账号 {username} 已选中好友 {friend_name} 发送消息")
                send_message_to_chat(page, username, friend_name, is_group=False)

        # 处理群聊
        if group_targets:
            logger.debug(f"账号 {username} 开始发送群聊消息")
...
def runTasks():
    ...
    try:
        ...
        for user in userData:
            ...
            group_targets = user.get("group_targets", [])
            complates[user["unique_id"]] = []  # 初始化该用户的已完成列表
            username = user.get("username", "未知用户")
            logger.info(f"开始处理账号 {username}")
            # 创建任务
            do_user_task(browser, username, cookies, targets, group_targets)
